...
(cl:defmethod cl:print-object ((obj Regexp-Match) stream)
  (cl:print-unreadable-object (obj stream :identity #t)
    (cl:format stream "Regexp-Match")))
              ...
              (or (not (string-cursor? (regexp-match-ref m1 i)))
                  ...
                  (and (string-cursor? (regexp-match-ref m1 (+ i 1)))
                       (string-cursor<? (regexp-match-ref m1 (+ i 1))
                                        (regexp-match-ref m1 i)))
                  (_(if (memq (+ i 1) non-greedy-indexes) #'not #'values)
                   (and
                    (string-cursor=? (regexp-match-ref m2 i)
                                     (regexp-match-ref m1 i))
...
(define (posse-for-each proc posse)
  (hash-table-walk posse (lambda (key val) (_proc val))))
(define (posse-every pred posse)
  (hash-table-fold posse (lambda (key val acc) (and acc (_pred val))) #t))
...
(define (state-matches? st str i ch start end matches)
  (let ((matcher (state-chars st)))
    (cond
     ...
     ((procedure? matcher)
      (_matcher str i ch start end matches))
     ((not matcher))
     (else
...
(cl:define-symbol-macro char-set$nonl
  (char-set-difference char-set$full (char-set #\newline)))
;;(cl:define-symbol-macro char-set$control (ucs-range->char-set 0 32))
(cl:define-symbol-macro char-set$word-constituent
        ...
        => (lambda (md)
             (let ((j (regexp-match-ref md 1)))
               (lp (if (and (string-cursor=? i j) (string-cursor<? j end))
                       ...
                       j)
                   j
                   (_kons (string-cursor->index str from) md str acc)))))
       (else
         (_finish (string-cursor->index str from) #f str acc))))))
...
(cl:define-symbol-macro re$grapheme (regexp 'grapheme))
